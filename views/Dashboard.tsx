...
import { Camera, X, Disc, Mic, Zap, Send, EyeOff, Eye, Cpu, HardDrive, Wifi, ChevronRight, ChevronLeft, Sparkles } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const triggerAura = () => {
    if (!connected) return;
    addLog("EXEC_PROTOCOL: SEND_AURA_WAVE", "success");
    mqttService.publishCommand(sessionId, { type: 'SEND_AURA', timestamp: new Date().toISOString() });
  }
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-black font-mono text-green-500">
      ...
      <div className="w-1/3 flex flex-col border-r border-gray-800 z-30 bg-black">
        ...
        <div className="p-4 bg-zinc-950 border-t border-gray-800 space-y-3">
           ...
           <div className="grid grid-cols-2 gap-2">
             ...
             <button onClick={triggerAura} disabled={!connected} className="flex items-center justify-center gap-2 py-2 text-[10px] font-bold border bg-green-950/20 border-green-900 text-green-400 hover:bg-white hover:text-black transition-all">
               <Sparkles size={14} /> SEND_AURA
             </button>
           </div>
        </div>
      </div>
      ...
      {/* Center: Map / QR */}
      <div className="flex-1 relative border-r border-gray-800 bg-black overflow-hidden">
        {targetCoords ? (
        ...
        <button 
          ...
          className="absolute right-0 top-1/2 -translate-y-1/2 z-50 bg-zinc-900 border border-gray-800 p-1 text-gray-500 hover:text-green-500 transition-colors"
        >
      ...
      <div className={`bg-zinc-950 flex flex-col z-30 transition-all duration-300 border-l border-gray-800 ${sidebarVisible ? 'w-1/4 translate-x-0' : 'w-0 translate-x-full overflow-hidden border-none'}`}>
        <div className="p-4 border-b border-gray-900 flex items-center justify-between bg-black whitespace-nowrap">
        ...
        {lastPayload ? (
          <div className="p-4 space-y-6 text-[11px] overflow-y-auto flex-1 custom-scrollbar">
            {/* HW Section */}
            <section className="space-y-2">
              <h4 className="text-gray-600 flex items-center gap-2 uppercase tracking-tighter"><Cpu size={12}/> Hardware_Stub</h4>
            ...
            {/* Network Section */}
            <section className="space-y-2">
              <h4 className="text-gray-600 flex items-center gap-2 uppercase tracking-tighter"><HardDrive size={12}/> Network_Trace</h4>
              <div className="bg-black/50 p-2 border border-gray-900 space-y-1">
            ...
            {/* Client Context */}
            <section className="space-y-2">
              <h4 className="text-gray-600 uppercase tracking-tighter">Client_Context</h4>
            ...
            <div className="pt-4 border-t border-gray-900">
               ...
               <button onClick={() => setLastPayload(null)} className="w-full py-2 border border-gray-800 text-gray-600 hover:text-red-500 hover:border-red-900 text-[9px] transition-colors uppercase">Flush Data</button>
            </div>
          </div>
        ) : (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center opacity-20">
            ...
            <div className="text-[10px] uppercase tracking-widest">Awaiting Link...</div>
          </div>
        )}
      </div>
